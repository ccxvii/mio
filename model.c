...
static const char *bone_vert_src =
	...
	"	for (int i = 0; i < 4; i++) {\n"
	"		position += SkinMatrix[int(index.x)] * vec4(att_Position, 1) * weight.x;\n"
	"		normal += SkinMatrix[int(index.x)] * vec4(att_Normal, 0) * weight.x;\n"
	"		index = index.yzwx;\n"
	"		weight = weight.yzwx;\n"
	"	}\n"
...
static const char *bone_vert2_src =
	"#version 120\n"
	"uniform mat4 Projection;\n"
	"uniform mat4 ModelView;\n"
	"uniform mat4 SkinMatrix[80];\n"
	...
	"varying vec3 var_Normal;\n"
	"void main() {\n"
	"	vec4 position = SkinMatrix[int(index.x)] * vec4(att_Position, 1) * weight.x;\n"
	"	position += SkinMatrix[int(index.y)] * vec4(att_Position, 1) * weight.y;\n"
	"	position += SkinMatrix[int(index.z)] * vec4(att_Position, 1) * weight.z;\n"
	"	position += SkinMatrix[int(index.w)] * vec4(att_Position, 1) * weight.w;\n"
	"	position = ModelView * position;\n"
	"	vec4 normal = SkinMatrix[int(index.x)] * vec4(att_Normal, 1) * weight.x;\n"
	"	normal += SkinMatrix[int(index.y)] * vec4(att_Normal, 1) * weight.y;\n"
	"	normal += SkinMatrix[int(index.z)] * vec4(att_Normal, 1) * weight.z;\n"
	"	normal += SkinMatrix[int(index.w)] * vec4(att_Normal, 1) * weight.w;\n"
	"	normal = ModelView * normal;\n"
	"	gl_Position = Projection * position;\n"
	"	var_Position = position.xyz;\n"
	"	var_Normal = normal.xyz;\n"
