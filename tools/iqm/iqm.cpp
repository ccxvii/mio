...
struct eanim
{
    ...
    eanim() : name(NULL), firstframe(0), fps(0), flags(0) {}
};
...
struct halfdata
{
    ...
    // TODO: implement proper conversion here
    template<class U>
    halfdata(const U &val) : val(ushort(val)) {}
};
...
void setupvertexarray(const vector<T> &attribs, int type, int fmt, int size)
{
    ...
    if(va.type >= IQM_CUSTOM) va.type = IQM_CUSTOM + sharestring(name);
...
void makeanims()
{
    ...
    loopv(eanims)
    {
        ...
        a.fps = ea.fps;
        a.flags = ea.flags;
        for(int j = a.firstframe, end = eanims.inrange(i+1) ? eanims[i+1].firstframe : eframes.length(); j < end; j++)
        {
...
bool loadiqe(const char *filename)
{
    ...
    while(f->getline(buf, sizeof(buf)))
    {
        char *c = buf;
        while(isspace(*c)) ++c;
        if(isalpha(c[0]) && isalpha(c[1]) && (!c[2] || isspace(c[2]))) switch(*c++)
        {
        case 'v':
            switch(*c++)
            {
            ...
            case 'n': enormals.add(Vec3(parseattribs4(c))); continue;
            case 'b': eblends.add(parseblends(c)); continue;
            case 'c': ecolors.add(parseattribs4(c)); continue;
...
//            if(emeshes.length()) parsename(c, emeshes.last().material);
        ...
        {
            if(eanims.length()) eanims.last().flags |= eanim::F_LOOP;
        }
...
bool loadmd5anim(const char *filename, const char *animname = NULL, int animflags = 0)
{
    stream *f = openfile(filename, "r");
    if(!f) return false;
    ...
    eanim &a = eanims.add();
    if(animname) a.name = getnamekey(animname);
    else
    {
        string name;
...
bool loadsmd(const char *filename, const char *animname = NULL, int animflags = 0)
{
    stream *f = openfile(filename, "r");
    if(!f) return false;
...
bool writeiqm(const char *filename)
{
    ...
    if(stringdata.length()) hdr.ofs_text = hdr.filesize; hdr.num_text = stringdata.length(); hdr.filesize += stringdata.length();
    ...
    hdr.num_vertexarrays = varrays.length(); if(varrays.length()) hdr.ofs_vertexarrays = hdr.filesize; hdr.filesize += varrays.length() * sizeof(vertexarray) + vdata.length();
    ...
    hdr.num_frames = poses.length() ? frames.length()/poses.length() : 0; hdr.num_framechannels = framesize; if(animdata.length()) hdr.ofs_frames = hdr.filesize; hdr.filesize += animdata.length() * sizeof(ushort); if(bounds.length()) hdr.ofs_bounds = hdr.filesize; hdr.filesize += bounds.length() * sizeof(float[8]);
    if(commentdata.length()) hdr.ofs_comment = hdr.filesize; hdr.num_comment = commentdata.length(); hdr.filesize += commentdata.length();
...
int main(int argc, char **argv)
{
    ...
    static const char *infiles[1024];
    int numinfiles = 0;
    ...
    for(int i = 1; i < argc; i++)
    {
        ...
        case 'o':
            ...
            break;
        case 'i':
            if(i + 1 < argc && numinfiles < int(sizeof(infiles)/sizeof(infiles[0]))) infiles[numinfiles++] = argv[++i];
            break;
        case 's':
        ...
        else if(numinfiles < int(sizeof(infiles)/sizeof(infiles[0]))) infiles[numinfiles++] = argv[i];
    }

    if(!outfile) fatal("no output file specified");
    else if(!numinfiles) fatal("no input files specified");

    if(escale != 1) printf("scale: %f\n", escale);

    for(int i = 0; i < numinfiles; i++)
    {
        const char *infile = infiles[i], *type = strrchr(infile, '.');
        if(!type) fatal("no file type");
		else if(!strcasecmp(type, ".md5mesh"))
        {
			if(loadmd5mesh(infile)) conoutf("imported: %s", infile);
			else fatal("failed reading: %s", infile);
		}
		else if(!strcasecmp(type, ".md5anim"))
		{
			if(loadmd5anim(infile)) conoutf("imported: %s", infile);
			else fatal("failed reading: %s", infile);
		}
        else if(!strcasecmp(type, ".iqe"))
        {
            if(loadiqe(infile)) conoutf("imported: %s", infile);
            else fatal("failed reading: %s", infile);
        }
        else if(!strcasecmp(type, ".smd"))
        {
            if(loadsmd(infile)) conoutf("imported: %s", infile);
            else fatal("failed reading: %s", infile);
        }
