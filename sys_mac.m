...
- (void) keyDown:(NSEvent *)evt
{
	...
	if (len && ![evt isARepeat])
		sys_hook_key_down(translate_key([str characterAtIndex: 0]), mod);
	for (i = 0; i < len; i++)
		sys_hook_key_char(translate_key([str characterAtIndex: i]), mod);
	[self setNeedsDisplay: YES];
}
...
- (void) keyUp:(NSEvent *)evt
{
	...
	int len = [str length];
	if (len)
		sys_hook_key_up(translate_key([str characterAtIndex: 0]), mod);
	[self setNeedsDisplay: YES];
}
...
- (void) applicationDidFinishLaunching:(NSNotification *)notification
{
	...
	rect = NSMakeRect(50, 100, 800, 600);
